...
function recursiveWalk(obj, callback) {
  function recurse(o) {
    if (Array.isArray(o)) {
      for (var i = 0; i < o.length; i++) recurse(o[i]);
...
const logger = new Logger(
  'C:/Users/emers/AppData/Roaming/Toon Boom Animation/Toon Boom Harmony Premium/2400-scripts/log.json',
);
...
function ProfileRegionSet(params) {
  ...
  layers.forEach(function (profileRegion) {
    ...
    if (ProfileSet.PROFILES.indexOf(regionName) === -1) return;
...
const DEFAULT_CHARACTER_PROFILES =
  'C:\\Users\\emers\\AppData\\Roaming\\Toon Boom Animation\\Toon Boom Harmony Premium\\2400-scripts\\characters';
...
function saveProfilesToJSON(params) {
  ...
  if (!savePath) throw Error('Failed to load profiles from file: ' + path);
...
class ProfileSet {
  ...
  static DEFAULT_CHARACTER_PROFILES =
    'C:\\Users\\emers\\AppData\\Roaming\\Toon Boom Animation\\Toon Boom Harmony Premium\\2400-scripts\\characters';
  static PROFILES = [
    'right_back',
    'right_3',
  ...
  constructor(path) {
    ...
    if (!fileContent) throw Error('Failed to load profiles from file: ' + path);
    ...
    objectForEach(profiles, function (profileName, profileData) {
      levelNames.forEach(function (seg) {
        levels[seg][profileName] =
          profileData[seg] && profileData[seg].artLayers ? profileData[seg].artLayers : {};
      });
    });
    this.levels = levels;
  ...
  getActiveNode() {
    ...
    if (selNode && selNode !== '') this.updateLastSelectedNode(selNode);
    else selNode = this.getLastSelectedNode();
    return selNode;
  }

  pasteProfile(selectedProfile, selectedLevel, transformations, options) {
    ...
    data.arts.forEach(function (c_artLayer) {
      ...
      if (!activeProfileData[artKey]) return;

      c_artLayer.layers.forEach(function (c_layer, i_layer) {
        if (activeProfileData[artKey].indexOf(i_layer) !== -1) layers.push(c_layer);
      });
...
class PasteProfileTool implements HarmonyTool {
  ...
  onMouseDown(ctx) {
    try {
      ...
      this.updatePasteProfilePreview(
        latestSelectedLevel.selectedProfile,
        currentLevel,
        srcFrameDrawingData,
      );
      ...
      this.snapDistance =
        typeof snapDistanceValue === 'number' && !isNaN(snapDistanceValue)
          ? snapDistanceValue
          : PasteProfileTool.DEFAULT_SNAP_DISTANCE;
  ...
  onMouseMove(ctx) {
    ...
    try {
      ...
      if (distance > this.CAP_DISTANCE) distance = this.CAP_DISTANCE;
      ...
      if (Array.isArray(previewPaths) && previewPaths.length > 0) {
        ...
        this._.DrawingView.scaleRecursive(
          previewPaths,
          { x: previewScale / 20, y: previewScale / 20 },
          center,
        );
        this._.DrawingView.rotateRecursive(previewPaths, previewAngle, center);
        this._.DrawingView.translateRecursive(
          previewPaths,
          this._.Math.subtract2d(previewCenter, center),
        );
      ...
      if (profilesData && profilesData.levels && selectedProfile && currentLevel) {
        var fileContent = this.GetFileContents(
          this.ProfileSet.DEFAULT_CHARACTER_PROFILES + '\\papyrus.json',
        );
        if (fileContent) {
          var json = JSON.parse(fileContent);
          if (
            json.profiles &&
            json.profiles[selectedProfile] &&
            json.profiles[selectedProfile][currentLevel] &&
            json.profiles[selectedProfile][currentLevel].bbox
          ) {
            bbox = json.profiles[selectedProfile][currentLevel].bbox as {
              x0: number;
              y0: number;
              x1: number;
              y1: number;
  ...
  onMouseUp(ctx) {
    ...
    if (
      ...
      typeof scene.metadatas === 'function' &&
      typeof scene.removeMetadata === 'function'
    ) {
      var metadatas = scene.metadatas();
      for (var i = 0; i < metadatas.length; i++) {
        ...
        if (
          ...
          if (meta) scene.removeMetadata(meta);
        }
      }
    }
    ...
    try {
      var transformations = [
        ...
        new this._.Transformations.Rotate(
          ctx.rotationAngle + this._.Math.degreesToRadians(90),
          ctx.center,
      ...
      MessageLog.trace(
        'Profile pasted at ' +
          JSON.stringify({ x: ctx.center.x, y: ctx.center.y }) +
          ' with scale ' +
          ctx.scale,
      );
      ...
      registerAction({
        ...
        callback: function () {
          try {
            ...
            var srcFrameDrawingData = srcFrame.getDrawingData();
            // Use new helper to update preview and get layersToPaste
            toolInstance.updatePasteProfilePreview(
              latestSelectedLevel.selectedProfile,
              currentLevel,
              srcFrameDrawingData,
            );
