...
class BaseTooltip extends QWidget {
  ...
  constructor() {
    ...
    this.setStyleSheet(
      'QWidget { background-color: #333; color: white; border-radius: 10px; padding: 4px; font-family: Arial; font-size: 10pt; }',
    );
...
class StringListWithButtons extends BaseTooltip {
  ...
  constructor(
    ...
    buttonCallback?: (index: number, text: string) => void,
  ) {
    super();
...
class ColumnDiscrepancyIndicator extends QPushButton {
  ...
  constructor() {
    ...
    this.setStyleSheet(
      'padding:0; margin:0; font-size:12px; min-width:0; min-height:0;max-width:18px; max-height:18px;',
    );
  }
}
      ...
      if (obj.param === key && obj.controllerType === 'Button') {
        obj['clicked()'].connect(() => {
          ...
          try {
            ...
            MessageLog.trace(
              'Error in button callback for param ' +
                key +
                ': ' +
                e.toString() +
                ' | ' +
...
class ColorPickerController {
    ...
    this.button['clicked()'].connect(
      wrapWithCatch(() => {
        ...
        var colorPicker = new G.Widgets.ColorPickerDialog({
          initialColor: color,
          onDragStart:
            G.Utils.bind(onDragStart, this) ||
            (() => {
              scene.beginUndoRedoAccum('Color picker drag');
            }),
          onDragEnd:
            G.Utils.bind(onDragEnd, this) ||
            (() => {
          ...
          onDragMove: (color) => {
            ...
            this.button.setStyleSheet(
              'background-color: rgb(' +
                ...
                ',' +
                rgb.b +
                ');  border: 3px solid #454545; border-radius: 10px;',
            );
  ...
  updateColor(newValues: { r: number; g: number; b: number }) {
    ...
    this.button.setStyleSheet(
      'background-color: ' + color.toHex() + '; border: 3px solid #454545; border-radius: 10px;',
    );
  }
}
...
class SliderController {
  ...
  constructor(params: Partial<SliderController>) {
    ...
    this.slider['valueChanged(int)'].connect(
      G.Utils.bind((value) => {
        // Custom logic before
    ...
    this.spinbox['valueChanged(int)'].connect(
      G.Utils.bind((value) => {
        let modifiedValue = this.modifier ? this.modifier(value) : value;
...
class LightingPage {
  ...
  toString() {
    return `Lighting Page<${this.lightingGroup.index}>`;
  }
  ...
  constructor(lightingGroup: LightingGroup, parent: QWidget) {
    ...
    initializeButtons(controlInterface.children(), {
      ...
      paste: () => {
        ...
        G.Utils.toast(
          `Pasted lighting data from metadata successfully!`,
          { x: 100, y: 100 },
          2000,
          '#4BB543',
          this.parentWindow,
    ...
    controlInterface.toolBox
      .widget(1)
      .verticalLayoutWidget.verticalLayout.addWidget(listToolBarWidget, 0, 0);
    controlInterface.toolBox
      .widget(1)
      .verticalLayoutWidget.verticalLayout.addWidget(listWidget, 0, 0);
    ...
    for (const layer of layers) {
      ...
      checkbox.setChecked(
        this.lightingGroup.passManager.isNodeInGroup(new oSelection(undefined, undefined, [layer])),
      );
      checkboxes.push(checkbox);

      checkbox.stateChanged.connect(function (state) {
        if (state === 0) (this.lightingGroup as LightingGroup).passManager.removeFromGroup([layer]);
        else (this.lightingGroup as LightingGroup).passManager.addToGroup([layer]);
      });
    }

    selectAll['clicked()'].connect(() => {
      checkboxes.forEach(
        wrapWithCatch(function (checkbox) {
          checkbox.setChecked(true);
          (this.lightingGroup as LightingGroup).passManager.addToGroup([
            layers[checkboxes.indexOf(checkbox)],
          ]);
        }),
      );
    });
    deselectAll['clicked()'].connect(() => {
      checkboxes.forEach(
        wrapWithCatch(function (checkbox) {
          ...
          (this.lightingGroup as LightingGroup).passManager.removeFromGroup([
            layers[checkboxes.indexOf(checkbox)],
          ]);
        }),
      );
  ...
  importLighting(path?: string) {
    ...
    this.update(output);
    MessageLog.trace(
      this.toString() + ' | ' + this.lightingGroup.toString() + ' imported succesfully !',
    );
  }
          ...
          case 'Color Picker': {
            ...
            controllerFrame.colorPicker = new ColorPickerController(
              ...
              function () {
                ...
                this.colors.red.column = G.Utils.getValueByPath(
                  this.lightingPage.lightingGroup.getColumns(),
                  this.param,
                ).r;
                this.colors.green.column = G.Utils.getValueByPath(
                  this.lightingPage.lightingGroup.getColumns(),
                  this.param,
                ).g;
                this.colors.blue.column = G.Utils.getValueByPath(
                  this.lightingPage.lightingGroup.getColumns(),
                  this.param,
                ).b;
          ...
          case 'Peg': {
            ...
            var pegController = new PegController({
              ...
              dragReleaseCallback: function (x, y) {
                ...
                MessageLog.trace(
                  `Set  ${this.lightingPage.lightingGroup.layer.nodePath} | ${param} to ${x}, ${y}`,
                );
              },
  ...
  update(value: any) {
    for (const [key, groupBox] of Object.entries(this.groupBoxes)) {
      ...
      for (const controllerFrame of controllerFrames) {
        ...
        const columnDiscrepancyIndicator =
          controllerFrame.columnDiscrepancyIndicator as ColumnDiscrepancyIndicator;

        if (columnDiscrepancyIndicator) {
          if (this.lightingGroup.multipleValueColumns[param]) {
            columnDiscrepancyIndicator.toast.setText(
              `Multiple values in selection for parameter: \n[${this.lightingGroup.multipleValueColumns[param].join(', ')}]`,
            );
            columnDiscrepancyIndicator.show();
          } else columnDiscrepancyIndicator.hide();
        }
        switch (controllerFrame.controllerType) {
          case 'Color Picker': {
...
class PresetListItemWidget extends QWidget {
  ...
  constructor(fileName, presetDir, lightingPage) {
    ...
    this.openButton['clicked()'].connect(
      G.Utils.bind(() => {
    ...
    this.applyButton['clicked()'].connect(() => {
      try {
        ...
        MessageLog.trace(
          'Error applying preset from file ' + this.fileName + ': ' + error.toString(),
        );
      }
    });
  }
...
function attachTooltip(widget: QWidget) {
  widget.enterEvent = wrapWithCatch(() => {
    if (!widget.tooltip) widget.tooltip = new G.TooltipToast('test');
  ...
  widget.leaveEvent = () => {
    if (widget.tooltip) widget.tooltip.hide();
...
class PopupPresetDialog extends QDialog {
  ...
  refreshPresetList() {
    try {
      ...
      for (let i = 0; i < fileNames.length; i++) {
        ...
        const itemWidget = new PresetListItemWidget(
          fileNames[i],
          this.presetDir,
          this.lightingPage,
        );
  ...
  addNotifiers() {
    try {
      ...
      this.myNotifier.currentFrameChanged.connect(
        G.Utils.bind(() => {
      ...
      this.myNotifier.selectionChanged.connect(
        G.Utils.bind(() => {
          try {
            ...
            MessageLog.trace(
              'Error in selectionChanged notifier: ' +
                error.toString() +
                ' | ' +
                error.lineNumber +
                ' |  ' +
      ...
      MessageLog.trace(
        'Error adding notifiers: ' + e.toString() + ' | ' + e.lineNumber + ' |  ' + e.fileName,
      );
    }
  }
  ...
  removeNotifiers() {
    try {
      ...
      MessageLog.trace(
        'Error removing notifiers: ' + e.toString() + ' | ' + e.lineNumber + ' |  ' + e.fileName,
      );
    }
  }
  ...
  private LightingGroupPicker(parentLayout: QVBoxLayout): QComboBox {
...

    {
      ...
      for (const lightingGroup of this.masterLightingController.getAllLightingGroups()) {
        options.push(
          `${PassManager.MAPPINGS[lightingGroup.index].symbol} ${lightingGroup.getName()}`,
        );
      }
...

    {
      ...
      this.currentLightingGroupEnabledBtn = new QPushButton(
        this.lightingGroup.isEnabled() ? '👀' : '🙈',
      );
      this.currentLightingGroupEnabledBtn.setFixedWidth(30);

      dropdownRow.addWidget(this.currentLightingGroupEnabledBtn, 0, 0);
      this.currentLightingGroupEnabledBtn['clicked()'].connect(
        bind(() => {
...

    {
      ...
      applyBtn['clicked()'].connect(
        bind(() => {
          ...
          G.Utils.toast(
            `Applied "${this.lightingGroup.getName()}" to ${currentSelection.getSelectSize()} frames`,
            { x: 100, y: 100 },
            1000,
            '#444444',
            this.parentWindow,
          );
          this.masterLightingController.setLightingGroupOfSelection(
            this.lightingGroup,
            currentSelection,
          );
        }, this),
...

    {
      ...
      clearSelectedBtn['clicked()'].connect(
        wrapWithCatch(
          bind(() => {
            G.Utils.toast(
              `Cleared lighting for ${new G.oSelection().getSelectSize()} frames`,
              { x: 100, y: 100 },
              1000,
              '#444444',
            );
  ...
  private setupPresetListSection(): Section {
    ...
    openDirectoryButton['clicked()'].connect(
      bind(() => {
  ...
  private setupDropdownHandler(dropdownBox: QComboBox) {
    dropdownBox['currentIndexChanged(int)'].connect(
      bind((index) => {
        ...
        for (var i = 0; i < this.masterLightingController.getAllLightingGroups().length; i++) {
          ...
          dropdownBox.setItemText(
            i,
            `${true ? PassManager.MAPPINGS[lightingGroup.index].symbol : '🔿'} ${lightingGroup.getName()} ${lightingGroup.isEnabled() ? '' : '🙈'}`,
          );
        }
      }, this),
    );
  ...
  private setupRenderPreviewRow(): Section {
...

    {
      ...
      toggleEnableAllBtn['clicked()'].connect(
        bind(() => {
...

    {
      ...
      var textValue = node
        .getAttr(this.masterLightingController.renderPreviewNode.nodePath, 1, 'scaling')
        .textValue();
      ...
      renderPreviewSlider.valueChanged.connect(
        bind((value) => {
          if (value === 0) {
            ...
            node.setTextAttr(
              this.masterLightingController.renderPreviewNode.nodePath,
              'scaling',
              1,
              scale,
            );
...

    {
      ...
      liveControlsEnabledCheckbox.stateChanged.connect(
        bind((state) => {
  ...
  private setupNodeEnabledSection(): Section {
    ...
    var nodeEnabledCheckboxes = {
      ...
      shadows: {
        nodes: [
          'Shadows_Peg',
          'Shadow_Apply_Transform',
          'Shadow_Blending_Multiply',
          'Shadow_Mask',
          'Lighting_Drawings/Drawing_Shadow',
        ],
      },
      rimlights: {
        nodes: [
          ...
          'Rimlight_Right_Peg',
          'Rimlight_Base',
          'Rimlight_Combine_Left_Right',
          'Extract_Outline',
          'Rimlight_Right_Mask',
          'Rimlight_Left_Mask',
          'Outline_Mask',
          'Lighting_Drawings/Drawing_Rimlight',
        ],
      },
      rbloom: {
    ...
    for (const [key, obj] of Object.entries(nodeEnabledCheckboxes)) {
      ...
      var isChecked = this.masterLightingController.getAllLightingGroups().every((group) => {
        return obj.nodes.every((nodePath) => {
          ...
          if (!node) MessageLog.trace('cant find ' + nodePath + ' in ' + group.layer.name);
      ...
      checkbox.stateChanged.connect(
        bind((state) => {
          G.Utils.toast(
            `${state === 2 ? 'Enabled' : 'Disabled'} ${key} | ${obj.nodes.length} nodes`,
            { x: 100, y: 100 },
            1000,
            '#444444',
            this.parentWindow,
    ...
    enableAllCheckbox.stateChanged.connect(
      bind((state) => {
        try {
          G.Utils.toast(
            `${state === 2 ? 'Enabled' : 'Disabled'} all lighting nodes`,
            { x: 100, y: 100 },
            1000,
            '#444444',
            this.parentWindow,
  ...
  for (var i = 0; i < buttons.length; ++i) {
    (function (btn) {
      ...
      button.setStyleSheet(
        'QPushButton { border: none; padding: 5px 10px; text-align: left; background-color: transparent; } QPushButton:hover { background-color: #a0a0a0; }',
      );
