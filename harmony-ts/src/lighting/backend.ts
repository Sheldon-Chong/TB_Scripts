...
include(specialFolders.userScripts + '/core/Renderer.js');
...
class MasterLightingController {
  ...
  getAppliedLightingGroup(selection: oSelection) {
    for (const lightingGroup of this.lightingGroups) {
      if (lightingGroup.passManager.isNodeInGroup(selection)) return lightingGroup;
    }
    return null;
  }
  ...
  getAppliedLightingGroups(selection: oSelection) {
    const appliedGroups = [];
    for (const lightingGroup of this.lightingGroups) {
      if (lightingGroup.passManager.isNodeInGroup(selection)) appliedGroups.push(lightingGroup);
    }
  ...
  constructor() {
    ...
    var lightingGroups = G.LayerManager.getNodeLayers()
      ...
      .map(
        bind(function (layer, index) {
          return new LightingGroup(
            layer,
            (index + 1) as LightingGroupRange,
            this,
            `Lighting Group ${index + 1}`,
          );
        }, this),
      );
  ...
  toString() {
    return `MasterLightingController<${this.lightingGroups.length} controllers>`;
  }
...
class PassManager {
  ...
  toString() {
    return `PassManager<index:${this.index}>`;
  }
  ...
  constructor(layer: NodeLayer, index: number, lightingGroup: LightingGroup) {
    ...
    this.passes = (this.layer.getChild('Drawing_Passes').getChildren() ?? [])
  ...
  setPass(passIndex: passRange, selection: oSelection, enabled: boolean) {
    this.passes[passIndex].setKeyFrame(
      selection,
      enabled ? PassManager.ENABLED : PassManager.DISABLED,
    );
  }

  setPasses(passIndexes: passRange[], selection: oSelection, enabled: boolean) {
    for (var i = 0; i < passIndexes.length; i++)
      this.passes[passIndexes[i]].setKeyFrame(
        selection,
        enabled ? PassManager.ENABLED : PassManager.DISABLED,
      );
  }
  ...
  addToGroup(nodes: NodeLayer[]) {
    ...
    nodes.forEach(function (currentNode) {
      ...
      var id = Timeline.createFrameMarker(
        currentNode.index,
        PassManager.MAPPINGS[self.index].marker,
        frame.current(),
      );
    });
  }
  ...
  isNodeInGroup(currentSelection?: oSelection): boolean {
    ...
    var passIndex = (parseInt(currentSelection.selectedNodes[0].name.substring(5)) -
      1) as passRange;
...
class LightingGroup {
  ...
  toString() {
    return `LightingGroup<${this.index}>`;
  }
  ...
  editLight() {
    this.editProperty('Light');
  }
  editMood() {
    this.editProperty('Mood');
  }
  editRimlight() {
    this.editProperty('Rimlight');
  }
  editShadow() {
    ...
    G.Utils.forEachLeafValue(attributeColumns, (value: any, path: string) => {
      if (value.constructor.name === 'Column') {
        ...
        if (Array.isArray(keyframeOrKeyframes)) {
          ...
          if (col.parent.name.indexOf('Drawing_') !== -1) return keyframeOrKeyframes[0];
          return Number(keyframeOrKeyframes[0]);
        } else {
          if (col.parent.name.indexOf('Drawing_') !== -1) return keyframeOrKeyframes;
  ...
  exportLighting() {
    var defaultDir =
      'C:\\Users\\emers\\Desktop\\Coding projects\\Python Bot\\ToonBoom_Automations\\test\\lightingPresets';
