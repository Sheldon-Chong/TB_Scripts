...
class ColorPickerButton extends QColorDialog {
  constructor(callback: (color: QColor) => void) {
    ...
    for (var child of this.children()) {
      if (child.text === '&Pick Screen Color') continue;
...
class ColorPickerDialog extends QDialog {
  ...
  constructor(params?: Partial<ColorPickerDialog>) {
    ...
    this.brightnessCanvas.setStyleSheet(
      'background: qlineargradient(x1:0, y1:0, x2:0, y2:1, stop:0 rgba(0,0,0,0), stop:1 rgba(0,0,0,255)); border: 1px solid black;',
    );
    ...
    this.circleIndicator.setStyleSheet(
      'background-color: white; border: 2px solid white; border-radius: ' + circleSize / 2 + 'px;',
    );
    ...
    this.hueBar.setStyleSheet(
      'background: qlineargradient(x1:0, y1:0, x2:1, y2:0, stop:0 #FF0000, stop:0.16 #FFFF00, stop:0.33 #00FF00, stop:0.5 #00FFFF, stop:0.66 #0000FF, stop:0.83 #FF00FF, stop:1 #FF0000); border: 1px solid black;',
    );
    ...
    this.colorPreview.setStyleSheet(
    ...
    this.colorPickerDialog = new _.Widgets.ColorPickerButton(function (color) {
      self.currentColor = Object._.ColorUtils.ColorObj.fromRgb(
        color.red(),
        color.green(),
        color.blue(),
      );
  ...
  updateAllUI() {
    ...
    this.saturationCanvas.setStyleSheet(
      'background: qlineargradient(x1:0, y1:0, x2:1, y2:0, stop:0 #FFFFFF, stop:1 ' + hueHex + ');',
    );
    ...
    this.circleIndicator.setGeometry(
      x - circleSize / 2,
      y - circleSize / 2,
      circleSize,
      circleSize,
    ...
    this.circleIndicator.setStyleSheet(
      'background-color: ' +
        hexColor +
        '; border: 2px solid white; border-radius: ' +
        circleSize / 2 +
        'px;',
    );
...
class PegController extends QWidget {
  ...
  constructor(params) {
    ...
    indicator.setStyleSheet(
      'background-color: #66CCFF; border: 2px solid white; border-radius: 5px;',
    );
    indicator.raise();
...
function SimpleColorPicker(colorChangedCallback?: (color: QColor) => void) {
  ...
  for (child of children) {
    child.mousePressEvent = function (event) {
      MessageLog.trace(
        'Child mouse press: ' + this.text + ' at (' + event.x() + ', ' + event.y() + ')',
      );
    };
  }
  ...
  colorCanvas.mousePressEvent = function (event) {
    isDragging = true;
    MessageLog.trace(
      '[DRAG START] Mouse pressed on canvas at: (' + event.x() + ', ' + event.y() + ')',
    );
  };
  ...
  colorCanvas.mouseReleaseEvent = function (event) {
    if (isDragging) {
      MessageLog.trace(
        '[DRAG RELEASE] Mouse released on canvas at: (' + event.x() + ', ' + event.y() + ')',
      );
      isDragging = false;
    }
  };
