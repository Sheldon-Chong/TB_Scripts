...
DrawingView.selectAll = function () {
  Action.perform('selectAll()', 'cameraView');
};

DrawingView.groupSelection = function () {
  Action.perform('onActionGroup()', 'cameraView');
};
...
function recursiveApply(obj, methodName, args) {
  ...
  while (stack.length > 0) {
    ...
    if (Array.isArray(current)) {
      for (var j = 0; j < current.length; j++) stack.push(current[j]);
      continue;
    }

    for (var key in current) if (current.hasOwnProperty(key)) stack.push(current[key]);
  }
}
...
function applyTransform(obj, pairs, methodName, args) {
  ...
  for (var i in pairs) fn(pairs[i]);
}

DrawingView.translateRecursive = function (obj, vector) {
  recursiveApply(obj, 'translate', [vector]);
};
DrawingView.scaleRecursive = function (obj, scaleVec, origin) {
  recursiveApply(obj, 'scale', [scaleVec, origin]);
};
DrawingView.rotateRecursive = function (obj, angleRad, pivot) {
  recursiveApply(obj, 'rotate', [angleRad, pivot]);
};
  ...
  var artsCompiled = data.arts.map(function (artLayer) {
    ...
    transformations.forEach(function (curr_Transformation) {
      if (curr_Transformation instanceof Scale)
        ...
        MessageLog.trace(
          'Layer size: w=' + (layerBbox.x1 - layerBbox.x0) + ', h=' + (layerBbox.y1 - layerBbox.y0),
        );
        MessageLog.trace('Layer center: x=' + layerCenter.x + ', y=' + layerCenter.y);
        MessageLog.trace('Combined center: x=' + center.x + ', y=' + center.y);
        MessageLog.trace(
          'Target size: x=' + curr_Transformation.size.x + ', y=' + curr_Transformation.size.y,
        );
...
DrawingView.pasteGroup = function (data, pasteTo, transformations, options) {
  ...
  var transformations = transformations || [new Position(new Vector2d(0, 0))];
