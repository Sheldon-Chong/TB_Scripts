  ...
  try {
    ...
    if (lastMarkerData) {
      try {
        ...
        const lastNodes = lastMarker.nodes
          .map(function (path: string) {
            return _G.LayerManager.getNodeLayer(path);
          })
    ...
    if (statusLabel)
      statusLabel.text =
        'Snapped to: ' + snappedFrame + (snappedFrame === currentFrame ? ' (Current)' : '');
...
function loadKeyFramesTo3DPath() {
  ...
  var openPath = QFileDialog.getOpenFileName(
    0,
    'Open Keyframes JSON',
    '',
    'JSON Files (*.json);;All Files (*)',
