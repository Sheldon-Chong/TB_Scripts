...
function frameTraverse() {
  function jumpToBoundary(direction: 'next' | 'previous') {
    ...
    const filtered = markers.filter((m) =>
      direction === 'next' ? m.frame > current : m.frame < current,
    );
    ...
    const target = filtered.sort((a, b) =>
      direction === 'next' ? a.frame - b.frame : b.frame - a.frame,
    )[0];
    MessageLog.trace(
      `${direction.charAt(0).toUpperCase() + direction.slice(1)} Boundary marker at frame: ${target.frame}`,
    );
