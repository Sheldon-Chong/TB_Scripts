  ...
  registerAction({
    ...
    callback: function (_) {
      try {
        ...
        MessageLog.trace(
          'All destination nodes have same port count: ' + allDestNodesHaveSamePortCount,
        );
        ...
        if (destinationNodes.length === 1) {
          ...
          if (savedNodes.length === 1) {
            ...
            if (
              allDestNodesHaveSamePortCount &&
              savedNodes.length === node.numberOfInputPorts(destinationNodes[0])
            ) {
