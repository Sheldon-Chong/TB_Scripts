...
render.frameReady.connect((frame, celImage) => {
  try {
    if (
      ...
      (Renderer.currentTask.nodes && Renderer.currentTask.nodes.length > 0)
    )
      return;
    const ext = Renderer.currentTask.outputFormat;
    const outputDir =
      Renderer.currentTask.outputPath || `${specialFolders.userScripts}/image_cache`;
...
render.nodeFrameReady.connect((frame, celImage, nodePath) => {
  try {
    if (
      !Renderer.currentTask ||
      !Renderer.currentTask.nodes ||
      Renderer.currentTask.nodes.length === 0
    )
      return;
    const ext = Renderer.currentTask.outputFormat;
    const outputDir =
      ...
      // Handle node-specific rendering
        ...
        task.nodes.forEach((nodePath) => {
          ...
          if (files.indexOf(imgName) !== -1) {
            ...
            MessageLog.trace(
              `Image for frame ${startFrame} not found in cache for node ${nodePath}.`,
            );
            task.onFinished('', task);
            if (task.completedNodes) task.completedNodes.push(nodePath);
          }
